...
define(['aura_base', 'aura_sandbox', 'aura_perms', 'eventemitter'], function(base, sandbox, permissions, EventEmitter) {
  ...
  core.hasPermission = function(action, subscriber, event) {
    ...
    // Uses the 'perm' -> 'on' / 'emit' namespace inside of the sandboxes pubsub to determine
    // permissions. This namespace is a facade the allows EventEmitter's behavior to pass thru
    // simply.
    //
    // If permissions allows namespace.ext.that to use listen via 'on',
    // ['namespace', 'ext', 'that'] into ['perm', 'on', 'namespace', 'ext', 'that']
    //
